...
// capability (Forgejo fills both source-control and docker-registry, so those needs share it).
export const resolveState = (intent: IntentSet, catalog: Catalog = defaultCatalog): DesiredStateGraph => {
    ...
    return compile(toNodeMap(emit(intent, { byNeed })));
};
