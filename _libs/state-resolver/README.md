...
- Derive the control-plane platform and the application-plane support stack (repos, CI, deployments, tunnel, routes, workspace runner).
...
- [src/platform.ts](src/platform.ts) / [src/app.ts](src/app.ts) / [src/route.ts](src/route.ts) / [src/workspace.ts](src/workspace.ts) — per-area node derivation (control plane, app plane, DNS routes, dev workspace runner).
