...
// Pick one reconciliation-target artifact. Deterministic for now: an explicit preferKey wins, otherwise
...
export const choose = (candidates: readonly Candidate[], preferKey?: string): Candidate => {
    if (candidates.length === 0) {
        throw new Error("no candidate reconciliation-target artifacts to choose from");
    }
