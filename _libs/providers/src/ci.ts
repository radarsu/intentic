...
const workflowYaml = (parsed: CiInputs): string => {
    const base = `${parsed.registry}/${parsed.adminUser}/${parsed.repoName}`;
    return [
        ...
        "      - uses: code.forgejo.org/docker/login-action@v3",
        "        with:",
        `          registry: ${parsed.registry}`,
        `          username: ${parsed.adminUser}`,
        `          password: \${{ secrets.${SECRET_REGISTRY} }}`,
        "      - uses: code.forgejo.org/docker/build-push-action@v6",
        "        with:",
        "          context: .",
        "          push: true",
        "          tags: |",
