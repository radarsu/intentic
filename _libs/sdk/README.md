...
**Key exports:** `defineCandidates` (build → every valid reconciliation-target artifact — the set
`intentic resolve` chooses from); `defineStack` (one-shot → a single `DesiredStateGraph`, auto-picking or via
