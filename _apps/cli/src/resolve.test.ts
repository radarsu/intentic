...
describe("loadIntent", () => {
    ...
    it("classifies secrets: externals are user-supplied, platform admin secrets are intentic-generated", async () => {
        ...
        // FORGEJO_ADMIN_PASSWORD / KOMODO_ADMIN_PASSWORD / KOMODO_WEBHOOK_SECRET are injected by the resolver's
        // platform layer and marked generated; the externals stay env (intentic can't invent them).
        expect(collectSecrets(graph)).toEqual({
            env: ["CLOUDFLARE_API_TOKEN", "HOST_SSH_KEY", "PRODUCTION_DATABASE_URL", "STAGING_DATABASE_URL"],
            generated: ["FORGEJO_ADMIN_PASSWORD", "KOMODO_ADMIN_PASSWORD", "KOMODO_WEBHOOK_SECRET"],
        });
    });
